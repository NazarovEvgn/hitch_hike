...
### ⚠️ Карта не загружается / тайлы не отображаются (VPN)

**Причина:** VPN блокирует доступ к 2GIS API серверам.

**Решение:**
1. **Отключите VPN** и перезагрузите страницу
2. Если VPN необходим, используйте российские VPN-серверы
3. Добавьте исключения для доменов 2GIS в настройках VPN:
   - `*.2gis.com`
   - `*.2gis.ru`
   - `*.api.2gis.com`

**Примечание:** Особенно часто проблемы возникают с VPN, использующими серверы США/Европы.
