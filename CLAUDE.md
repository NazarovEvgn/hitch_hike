...
*Клиенты (passwordless OTP):*
- Любой номер телефона (в MVP режиме код будет в логах backend)
- Пример: +79991234567 → проверить OTP код в консоли API
...
- **Passwordless Authentication для клиентов:**
  - OTP-вход через SMS (MVP: код в логах)
  - Автоматическая регистрация по номеру телефона
  - Единая страница входа в Consumer App
  - Разделение auth flow: клиенты (OTP) vs бизнес (email/password)
